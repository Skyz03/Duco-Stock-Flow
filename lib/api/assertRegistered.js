...
/**
 * Looks up pcs_per_box for a pack product. Returns 1 if not found (safe fallback).
 * @param {import("@supabase/supabase-js").SupabaseClient} supabase
 * @param {string} productCode
 * @returns {Promise<number>}
 */
export async function getPackPcsPerBox(supabase, productCode) {
  const { data } = await supabase
    .from("pack_products")
    .select("pcs_per_box")
    .eq("product_code", productCode)
    .maybeSingle();
  return data?.pcs_per_box ?? 1;
}
